...
async function testH2(
  ...
  try {
    await client.dispatch({
      ...
      modelSelection: { provider: "codex", model: H2_RESUME_MODEL },
      runtimeMode: "full-access",
      interactionMode: "default",
      createdAt: new Date().toISOString(),
    });
...
async function testH3(client: T3Client, projectId: string): Promise<HypothesisResult> {
  ...
  try {
    await client.dispatch({
      ...
      modelSelection: { provider, model: provider === "codex" ? "gpt-5.3-codex" : "auto" },
      runtimeMode: "full-access",
      branch: null,
      worktreePath: null,
      createdAt: now,
...
async function testH4(client: T3Client, projectId: string): Promise<HypothesisResult> {
  ...
  try {
    await client.dispatch({
      ...
      modelSelection: { provider: "opencode", model: "auto" },
      runtimeMode: "full-access",
      branch: null,
      worktreePath: null,
      createdAt: now,
